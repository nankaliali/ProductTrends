...
python crawler/crawler.py --product_urls data/product_urls_almarsa_2.txt --config configs/almarsa-gourmet.json
echo "Crawler finished for Almarsa Gourmet."

echo "Running the crawler for LULU Hypermarket..."
python crawler/crawler.py --product_urls data/product_urls_lulu_2.txt --config configs/lulu_config.json
echo "Crawler finished for LULU Hypermarket."

echo "Running the crawler for Muscat Bakery..."
python crawler/crawler.py --product_urls data/product_urls_muscat_2.txt --config configs/muscat-bakery.json
