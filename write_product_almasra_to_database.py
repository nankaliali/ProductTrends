...
def extract_page_content(
    ...
    for element in config:
        ...
        if element["name"] == "price":
            product_content[element["name"]] = element_content[0].replace("﷼", "")
        else:
...
def main():
    ...
    for line in lines:
        ...
        if "###" in line:
            ...
            product_dict[category].append(line.strip())

    session = manager.get_session()

    try:
        for category_config in product_dict:

            for url in product_dict[category_config]:
                try:
                ...
                try:
                    manager.add_hierarchical_category(
                        category_config.split(","), session
                    )
                    category_id = manager.get_category_by_name(
                        category_config.split(",")[-1], session
                    ).id
                    ...
                    if content.get("image"):
                        ...
                        save_path = os.path.join(
                            "images_almasra",
                            f"{image_filename}_{unique_substring}.jpeg",
                        )
                    ...
                    organization_id = manager.add_organization(
                        organization_name, session
                    )
